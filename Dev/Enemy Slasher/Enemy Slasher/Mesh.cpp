...
vector<CMesh*> CDynamicShapeMesh::DynamicShaping(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, float fTimeElapsed, XMFLOAT4X4& mxf4x4ThisMat, CDynamicShapeMesh* pCutterMesh, XMFLOAT4X4& xmf4x4CutterMat, CutAlgorithm DynamicShapeAlgorithm)
{
	...
	case CutAlgorithm::Graph:
		return DynamicShaping_Graph(pd3dDevice, pd3dCommandList, fTimeElapsed, mxf4x4ThisMat, pCutterMesh, xmf4x4CutterMat);
	default:
		break;
	}
...
// 외곽선 탐색 함수
// 모든 edge들을 edgeMap에 담음
void CDynamicShapeMesh::FindBoundaryEdges(const std::vector<std::pair<CEdge, CEdge>>& edges, EdgeMap& edgeMap)
{
...
std::vector<CDynamicShapeMesh::CEdge> CDynamicShapeMesh::ExtractBoundaryLoop(EdgeMap& edgeMap)
{
	std::vector<CEdge> boundary;
	auto it = edgeMap.begin();
	boundary.push_back(it->first);
	CEdge current = it->first;
...
void CDynamicShapeMesh::TriangulateBoundary(const std::vector<CEdge>& boundary, std::vector<CVertex>& fillVertices, std::vector<UINT>& fillIndices, XMFLOAT3& normal)
{
	...
	CVertex centerVertex(center, normal, XMFLOAT2(0.5f, 0.5f));
	...
	for (size_t i = 0; i < boundary.size(); ++i) {
		newVertex1.m_xmf3Vertex.x = boundary[i].x;
		newVertex1.m_xmf3Vertex.y = boundary[i].y;
		newVertex1.m_xmf3Vertex.z = boundary[i].z;

		newVertex2.m_xmf3Vertex.x = boundary[(i + 1) % boundary.size()].x;
		newVertex2.m_xmf3Vertex.y = boundary[(i + 1) % boundary.size()].y;
		newVertex2.m_xmf3Vertex.z = boundary[(i + 1) % boundary.size()].z;

		AddTriangle(fillVertices, fillIndices, centerVertex, newVertex1, newVertex2, normal);
	}
}
...
void CDynamicShapeMesh::HandleTriangleCut(
	...
	std::vector<std::pair<CEdge, CEdge>>& edges)
{
	...
	// map float 해쉬 문제로 int형으로 체크 중.
	// 자연스러운 절단 면 구성을 위해 절단 평면 위의 점은 int로 casting
	for (auto& intersection : intersections) {
		intersection.m_xmf3Vertex.x = (int)intersection.m_xmf3Vertex.x;
		intersection.m_xmf3Vertex.y = (int)intersection.m_xmf3Vertex.y;
		intersection.m_xmf3Vertex.z = (int)intersection.m_xmf3Vertex.z;
	}
	...
	if (above.size() == 1 || below.size() == 1) {
		...
		// 일부 삼각형의 경우 한 면이 비기 때문에 모든 정점을 대상으로 삼각형 생성 (중복 면이 생겨 비효율적)
		if (above.size() == 1) {
		...
		edges.push_back({ intersections[0].GetVertex() ,intersections[1].GetVertex() });
	}
}
...
vector<CMesh*> CDynamicShapeMesh::DynamicShaping_Graph(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, float fTimeElapsed, XMFLOAT4X4& xmf4x4ThisMat, CDynamicShapeMesh* pCutterMesh, XMFLOAT4X4& xmf4x4CutterMat)
{
	...
	if (CCutterMesh* pCutter = dynamic_cast<CCutterMesh*>(pCutterMesh)) {
		...
		std::vector<std::pair<CEdge, CEdge>> vEdges; // 새로운 vertex 들의 edge vector
...
CCubeMeshTextured::CCubeMeshTextured(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, float fWidth, float fHeight, float fDepth) : CMesh(pd3dDevice, pd3dCommandList)
{
	...
	CVertex pVertices[36];
	int i = 0;

	pVertices[i++] = CVertex(XMFLOAT3(-fx, +fy, -fz), XMFLOAT2(0.0f, 0.0f));
	pVertices[i++] = CVertex(XMFLOAT3(+fx, +fy, -fz), XMFLOAT2(1.0f, 0.0f));
	pVertices[i++] = CVertex(XMFLOAT3(+fx, -fy, -fz), XMFLOAT2(1.0f, 1.0f));

	pVertices[i++] = CVertex(XMFLOAT3(-fx, +fy, -fz), XMFLOAT2(0.0f, 0.0f));
	pVertices[i++] = CVertex(XMFLOAT3(+fx, -fy, -fz), XMFLOAT2(1.0f, 1.0f));
	pVertices[i++] = CVertex(XMFLOAT3(-fx, -fy, -fz), XMFLOAT2(0.0f, 1.0f));
...
// ------------------------------- FBX V3 -----------------------------------
CFBXMesh::CFBXMesh(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, CFbx_V3::MeshData* pMeshData, UINT nStride) : CDynamicShapeMesh(pd3dDevice, pd3dCommandList)
{
	using namespace CFbx_V3;
	vector<VertexData>* vertexList = &pMeshData->m_vVertices;

	m_nVertices = vertexList->size();		// 꼭지점 개수
	m_nStride = nStride;					// Vertex의 크기
	m_d3dPrimitiveTopology = D3D_PRIMITIVE_TOPOLOGY_TRIANGLELIST;
	...
	if (nStride == sizeof(CVertex)) {
		CVertex* newVertices = new CVertex_Skining[m_nVertices];
		for (int i = 0; i < m_nVertices; ++i) {
...
CHpBarMesh::CHpBarMesh(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, float fWidth, float fHeight) : CMesh(pd3dDevice, pd3dCommandList)
{
	...
	// 왼쪽 상단
	pVertices[0] = VertexWithTexCoord(XMFLOAT3(-fx, +fy, 0.0f), XMFLOAT2(0.0f, 1.0f)); // 변경된 UV 좌표
	// 왼쪽 하단
	pVertices[1] = VertexWithTexCoord(XMFLOAT3(-fx, -fy, 0.0f), XMFLOAT2(0.0f, 0.0f)); // 변경된 UV 좌표
	// 오른쪽 상단
	pVertices[2] = VertexWithTexCoord(XMFLOAT3(+fx, +fy, 0.0f), XMFLOAT2(1.0f, 1.0f));

	// 오른쪽 상단
	pVertices[3] = VertexWithTexCoord(XMFLOAT3(+fx, +fy, 0.0f), XMFLOAT2(1.0f, 1.0f));
	// 왼쪽 하단
	pVertices[4] = VertexWithTexCoord(XMFLOAT3(-fx, -fy, 0.0f), XMFLOAT2(0.0f, 0.0f)); // 변경된 UV 좌표
	// 오른쪽 하단
	pVertices[5] = VertexWithTexCoord(XMFLOAT3(+fx, -fy, 0.0f), XMFLOAT2(1.0f, 0.0f)); // 변경된 UV 좌표
...
C2DTextureMesh::C2DTextureMesh(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, float fWidth, float fHeight) : CMesh(pd3dDevice, pd3dCommandList)
{
	m_nVertices = 6;
	m_nStride = sizeof(VertexWithTexCoord);
	m_nOffset = 0;
	...
	// 오른쪽 상단
	pVertices[0] = VertexWithTexCoord(XMFLOAT3(+fx, +fy, 0.0f), XMFLOAT2(1.0f, 0.0f));
	// 왼쪽 하단
	pVertices[1] = VertexWithTexCoord(XMFLOAT3(-fx, -fy, 0.0f), XMFLOAT2(0.0f, 1.0f));
	// 왼쪽 상단
	pVertices[2] = VertexWithTexCoord(XMFLOAT3(-fx, +fy, 0.0f), XMFLOAT2(0.0f, 0.0f));
	...
	// 오른쪽 상단
	pVertices[5] = VertexWithTexCoord(XMFLOAT3(+fx, +fy, 0.0f), XMFLOAT2(1.0f, 0.0f));
