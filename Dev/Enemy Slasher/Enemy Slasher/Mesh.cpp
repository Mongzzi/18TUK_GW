...
CHpBarMesh::CHpBarMesh(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, float fWidth, float fHeight) : CMesh(pd3dDevice, pd3dCommandList)
{
	...
	// 왼쪽 상단
	pVertices[0] = VertexWithTexCoord(XMFLOAT3(-fx, +fy, 0.0f), XMFLOAT2(1.0f, 1.0f)); // 변경된 UV 좌표
	// 왼쪽 하단
	pVertices[1] = VertexWithTexCoord(XMFLOAT3(-fx, -fy, 0.0f), XMFLOAT2(1.0f, 0.0f)); // 변경된 UV 좌표
	// 오른쪽 상단
	...
	// 왼쪽 하단
	pVertices[4] = VertexWithTexCoord(XMFLOAT3(-fx, -fy, 0.0f), XMFLOAT2(1.0f, 0.0f)); // 변경된 UV 좌표
	// 오른쪽 하단
	pVertices[5] = VertexWithTexCoord(XMFLOAT3(+fx, -fy, 0.0f), XMFLOAT2(0.0f, 0.0f)); // 변경된 UV 좌표
